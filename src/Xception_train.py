...
def run_task_eager(args):
    ...
    # learning_rate = tf.train.piecewise_constant(step_counter, [4, 4],
    ...
    # check_point.restore(os.path.join(args.output_dir, 'model2', 'cpkt-3'))  # 存在就恢复模型(可不使用)
    check_point.restore(tf.train.latest_checkpoint(os.path.join(args.output_dir, 'model2')))
    # 7. 训练、评估
