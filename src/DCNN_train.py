    ...
    for batch, (audios, labels) in enumerate(dataset):  # 遍历一次数据集
        with tfc.summary.record_summaries_every_n_global_steps(
            ...
            with tf.GradientTape() as tape:
                audios = tf.reshape(audios, (args.batch_size, 128, 157, 2))
                # audios = audios.numpy()
...
def run_task_eager(args):
    ...
    # learning_rate = tf.train.piecewise_constant(step_counter, boundaries=[40, 60], values=[0.005, 0.0005, 0.00005])
    ...
    check_point.restore(os.path.join(args.output_dir, 'model4', 'cpkt-29'))  # 存在就恢复模型(可不使用)
